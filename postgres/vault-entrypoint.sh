...
export POSTGRES_USER=$(echo "$SECRET" | jq -r '.data.data.username')
export POSTGRES_PASSWORD=$(echo "$SECRET" | jq -r '.data.data.password')
export POSTGRES_DB=$(echo "$SECRET" | jq -r '.data.data.database')
